...
Event types typically consist of (1) a view shown within the tags application that allows users to modify settings for the event and (2) a library module emitted within the tags runtime library to interpret the settings and watch for a certain activity to occur.
...
Action types typically consist of (1) a view shown within the tags application that allows users to modify settings for the action and (2) a library module emitted within the tags runtime library to interpret the settings and perform an action.
...
Data element types typically consist of (1) a view shown within the tags application that allows users to modify settings for the data element and (2) a library module emitted within the tags runtime library to interpret the settings and retrieve pieces of data.
...
The extension configuration consists of a view component that will export settings that are then emitted within the tags runtime library as a plain object.
...
Each library module (the logic to be emitted within the tags runtime library) must be its own file whose content follows the [CommonJS module standard](http://wiki.commonjs.org/wiki/Modules/1.1.1). For example, if we're building a "send beacon" action type, we must have a file that contains the logic that sends the beacon. The content of this file will be emitted within the tags runtime library. You might call it `sendBeacon.js`. The location of the file in the directory is not important since `extension.json` will describe where it is located.
